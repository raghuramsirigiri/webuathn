...
let verifyAuthenticatorAttestationResponse = (webAuthnResponse) => {
    ...
    // console.log("ctap", ctapMakeCredResp.fmt);
    // console.log("resp", attestationBuffer);
    // console.log(parseMakeCredAuthData(ctapMakeCredResp.authData));
    ...
    if(ctapMakeCredResp.fmt === 'fido-u2f') {
        ...
        let authrDataStruct = parseMakeCredAuthData(ctapMakeCredResp.authData);
        console.log(authrDataStruct.COSEPublicKey);
    ...
    /* ----- Verify signature ENDS ----- */
        ...
        console.log(response.authrInfo);
...
let verifyAuthenticatorAssertionResponse = (webAuthnResponse, authenticators) => {
    ...
    // console.log(authr);
    console.log("assertion ---- web auth response",webAuthnResponse);
    ...
    if(authr.fmt === 'fido-u2f') {
        ...
        let publicKey = base64url.toBuffer(authr.publicKey);
    ...
    //     // let attestationStruct = cbor.decodeAllSync(authenticatorData)[0];
    ...
    //     let clientDataHashBuf = hash2('sha256', base64url.toBuffer(webAuthnResponse.response.clientDataJSON));
    //     let nonceBase     = Buffer.concat([authenticatorData, clientDataHashBuf]);
    //     let nonceBuffer   = hash2('sha256', nonceBase);
    //     let expectedNonce = nonceBuffer.toString('base64');
    ...
    //     // let signatureBaseBuffer = Buffer.from(jwsParts[0] + '.' + jwsParts[1]);
    //     let certificate         = ASN1toPEM(base64url.toBuffer(authr.publicKey));
    //     let signatureBuffer     = base64url.toBuffer(SIGNATURE);
